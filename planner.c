...
// const char *pathtype_to_string(NodeTag pathtype);
// static char *pathtype_to_string(NodeTag pathtype)
...
PlannedStmt * standard_planner(Query *parse, const char *query_string, int cursorOptions,ParamListInfo boundParams)
{
	...
	if (cursorOptions & CURSOR_OPT_FAST_PLAN)
	{
		 ...
		 * tuple_fraction是PostgreSQL优化器用来返回结果集大小的一个比例参数，主要
		 * 用于非游标查询（如普通的select）
		 * 0.0：意味着用户只需要一个结果元组（类似LIMIT 1）
		 * 1.0：意味着用户需要整合结果集
		 * 介于两者之间的值，表示用户期望部分结果集
		 * cursor_tuple_fraction是一个专门针对游标（cursor）查询的参数，它的功能
		 * 类似于tuple_fraction,但针对的是游标的使用场景
		 * 
		 */
	...
	/* Select best Path and turn it into a Plan 从优化器信息中的upper_rel数据中获取用以表示查询优化器生成的最终关系   */

	final_rel = fetch_upper_rel(root, UPPERREL_FINAL, NULL);

	/*通过对RelOptInfo中访问路径两两比较。获取成本最低的访问路径，并最终确定最优路径best_path并生成基本计划树，调用create_plan*/
...
PlannerInfo *subquery_planner(PlannerGlobal *glob, Query *parse,PlannerInfo *parent_root,bool hasRecursion, double tuple_fraction)
{
	 ...
	 * 处理where子句和join/on子句中的ANY和EXISTS类型的子链接
	 */
	 ...
	 * query. 提升子查询，当子查询仅仅是一个简单的扫描或者连接时，就会把子查询或者子
	 * 查询的一部分合并到父查询中已进行优化
	 */
	 ...
	 * preprocess_expression预处理表达式
	 * 该函数采用递归扫描的方式处理PlannerInfo结构体里面保存的目标属性、HAVING子句、OFFSET子句、LIMIT子句和连接树jointree
	 * 
	 */
	 ...
	 * //生成计划树的主函数：调整Group BY 与 Order BY，
		//调用preprocess_targetlist预处理INSERT、UPDATE、DELETE和FOR UPDATE情况下的目标属性
		//调用query_planner函数一个基本查询创建路径并获得cheapest_path(代价最低执行路径)，
		再调用get_cheapest_fractional_path_for_pathkeys函数获得sorted_path(对排序最优的路径);
	 ...
	 */
	set_cheapest(final_rel);  //这两个函数在 PostgreSQL 中用于选择代价最小的路径。它们会比较候选路径的代价，并选择代价最小的路径作为最终执行计划的一部分。
...
const  char *pathtype_to_string(NodeTag pathtype){
    switch (pathtype)
    {
        ...
        default:              return "UnknownPathType";
    }
}
 ...
 *     给定关系，找到成本最低的访问路径，该路径返回预期设定元组数(根据输入的RelOptInfo数据结构找到成本最低的访问路径)
 *
 * We interpret tuple_fraction the same way as grouping_planner. 
 * 我们以与 grouping_planner 相同的方式解释 tuple_fraction。
 *
 * We assume set_cheapest() has been run on the given rel.
 * 我们假设已在给定的关系上运行过 set_cheapest()。
 */
Path *get_cheapest_fractional_path(RelOptInfo *rel, double tuple_fraction)
{
	...
	foreach (t, rel->pathlist)
	{
		Path *path = (Path *)lfirst(t);

		elog(LOG, "候选者:"
				   " pathtype=%s,parallel_aware=%s, parallel_safe=%s, "
				   " parallel_workers=%d, startup_cost=%.2f, total_cost=%.2f, rows=%.0f",
			 pathtype_to_string(path->pathtype),
			 path->parallel_aware ? "true" : "false",
			 path->parallel_safe ? "true" : "false",
	...
	if (tuple_fraction <= 0.0){
        ...
        elog(LOG,"bestpath:tuple_fraction <= 0.0,all tuples will be retrieved."
		                       "  pathtype=%s,parallel_aware=%s, parallel_safe=%s, "
		                   "  parallel_workers=%d, startup_cost=%.2f, total_cost=%.2f, rows=%.0f",
							 pathtype_to_string(best_path->pathtype),
							 best_path->parallel_aware ? "true" : "false",
							 best_path->parallel_safe ? "true" : "false",
	...
	if (tuple_fraction >= 1.0 && best_path->rows > 0)
		tuple_fraction /= best_path->rows; //为了统一计算，需要将绝对元组数转化为比例：
	...
	foreach(l, rel->pathlist)
	{
		...
		if (path == rel->cheapest_total_path ||
			compare_fractional_path_costs(best_path, path, tuple_fraction) <= 0)   //比较路径的costs
			continue;
